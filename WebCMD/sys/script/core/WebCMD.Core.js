...
$(document).on('paste', function (e) {
    //TODO: May add a image paste or so too ...
...
function handleKeyPressEvent(e) {
   ...
   // if (obj.id != CmdInputHelper.ClientID && obj.tagName == 'INPUT' || obj.tagName == 'TEXTAREA') return true;
    ...
    else if (keyCode == Key_Backspace) removeLastPrefix();
    else if (keyCode == Key_Tab);

    //HANDLE: Arrow-keys
    else if (keyCode == Key_ArrowLeft) keyArrowLeft();
    else if (keyCode == Key_ArrowRight) keyArrowRight();
    else if (keyCode == Key_ArrowUp) keyArrowUp();
    else if (keyCode == Key_ArrowDown) keyArrowDown();

    //HANDLE: Edit-keys
    else if (keyCode == Key_Ins) keyIns();
    else if (keyCode == Key_Del) removeFirstPostfix();
    else if (keyCode == Key_Home) keyHome();
    else if (keyCode == Key_End) keyEnd();
    else if (keyCode == Key_PageUp) { scroll = false; allowDefaultBehaviour = true; }
    else if (keyCode == Key_PageDown) { scroll = false; allowDefaultBehaviour = true; }

    //HANDLE: Key combinations
    else if (ctrlKey && charCode == 'v'.charCodeAt(0)) { allowDefaultBehaviour = true; }
    else if (ctrlKey && charCode == 'c'.charCodeAt(0)) { allowDefaultBehaviour = true; }

    //HANDLE: Extra "sausage" for MAC(faggots)
    else if (metaKey && charCode == 'v'.charCodeAt(0)) { allowDefaultBehaviour = true; }
    else if (metaKey && charCode == 'c'.charCodeAt(0)) { allowDefaultBehaviour = true; }
...
function calcHeaderMargins() {
    var h1 = WindowHeader.get().scrollHeight;
    WindowContent.get().style.marginTop = h1 + 'px';

    var h2 = ConsoleConInfo.get().scrollHeight;
    ConsoleDebug.get().style.marginTop = h2 + 'px';
