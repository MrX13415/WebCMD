    ...
    function _processResponse(rsdata) {
        ...
        if (eventArgs.length >= 4) {
            ...
            if (property != "innerHTML") {
                ...
                var useLongWait = false;        //wait longer then usual. (usualy after a block)
                ...
                (function type(noEnd) {
                    ...
                    if (char === '&') isCharTag = true;
                    //if the next special char is not a '#' 
                    else if (isCharTag && char.match(/[^A-Za-z0-9#]/g)) isCharTag = false;

                    if (isHtmlTag || isCharTag) return type();
