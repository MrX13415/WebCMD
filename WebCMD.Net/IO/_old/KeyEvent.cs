...
namespace WebCMD.Net.IO
{
    ...
    public class KeyEvent : ServerRequest
    {
        ...
        {
            return String.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}:{7}:{8}", RequestHandler.EventIdentString, TargetElementID, RequestTypeID, EventType, CtrlKeyDown, AltKeyDown, ShiftKeyDown, MetaKeyDown, KeyCode);
        }
    }
}